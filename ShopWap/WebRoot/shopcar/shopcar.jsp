...
<div class="m" id="m">
	...
	<div class="mc cart-list tl">
		<ul>
		<c:if test="${productNum==0}">
			<div class="mc">
				<div class="icon-cart">
					<span></span>
					<p>您的购物车暂时没有商品，去<a href='/ShopWap/index.jsp'>首页&gt;&gt;</a>挑选您喜欢的商品</p>
				</div>
			</div>
		</c:if>
		<c:if test="${productNum!=0}">
			<c:forEach items="${shopcarlist}" var="item">
				...
				<div class="clear"> 
					<div class="p-img p-img-v1 fl">
			     	<!--<input type="checkbox" class="input-chk"  checked  onclick="javascript:changeSelected(645456,1,this)"> -->
						<a href='/ShopWap/items/items.do?id=${item.itemId }' class="pay-img"><img width="50" alt="${item.name }" src="${item.img }"/></a>
					</div>  
					<div class="fl fl-v1">
						<p class="p-name"><a href='#'>${item.name }</a></p>
						<p class="p-num gray">编号：${item.itemId}</p>
						<p class="p-price">单价：<span class="red">￥${item.price }</span></p> 
						<p class="p-price2"></p>
						<p class="p-amount">数量：1 &nbsp;&nbsp;&nbsp;
							<a href="/ShopWap/shopcar/shopcar.do?id=${item.id}&action=delete"" class="btn">删除</a>
							<a href="/ShopWap/shopcar/shopcar.do?id=${item.id}&action=submit" class="btn">提交</a>
						</p>
					</div>  	
				</div>
				<p class="p-price2"></p>
				...
				<div class="pay-tip">
					<p class="common-border">原始金额：￥<span id="cart_oriPrice">${totalPrice }</span>- 返现：￥<span id="cart_rePrice">0.0</span></p>          
					<p>实际金额：<span class="red">￥</span><span class="red" id="cart_realPrice">${totalPrice}</span></p>
				</div>
				
				<div class="back"><a href='/ShopWap/index.jsp'><span></span>继续购物</a></div>
