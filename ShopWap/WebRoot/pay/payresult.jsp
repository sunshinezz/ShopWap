...
<html>
	...
	<body id="body">
		...
		<div class="order-tip">
			...
			<DIV class="o-mt">
			  <H2> <c:if test="${state==0 }">支付失败</c:if>
			  <c:if test="${state==1 }">支付成功</c:if></H2>
			</DIV>
			...
			<DIV id="order02" class="m m5">
			  ...
			  <DIV class="mc">
				...
				<form name="select" action="/ShopWap/pay/pay.do" method="post">
				  <input type="hidden" name="action" value="torecharge"/>
				  <TABLE border="0" cellSpacing="0" cellPadding="0" width="100%">
					...
					<TBODY id="tb-820609066" class="parent-815410507">
					  ...
					  <TR id="track820609066" oty="0">
						...
						<TD>￥${count}</TD>
						<TD><c:if test="${state==0 }">余额不足，支付失败，当前余额：￥${balance}</c:if>
						<c:if test="${state==1 }">支付成功，当前余额：￥${balance}</c:if>
						</TD>
					  </TR>
				  ...
				  <a><p><input type="submit" value="去充值" class="sub_btn"></p></a>
				  </form>
				</DIV>
			  </DIV>
			</DIV>
		...
		<div class="pay-tip">
		    <p>请在24小时内完成支付，过时订单将会被自动取消。<br/></p>
		</div>
