				...
				<c:if test="${item.state!=3}">
					...
					<div class="clear"> 
						<div class="p-img p-img-v1 fl">
							<a href='/ShopWap/items/items.do?id=${item.id }' class="pay-img"><img width="50" alt="${item.name}" src="${item.img}"/></a>
						</div>
						<div class="fl fl-v1">
							<p class="p-name"><a href='#'>${order.name}</a></p>
							<p class="p-num gray">编号：${order.id}</p>
							<p class="p-price">单价：<span class="red">￥${item.price}</span></p> 
							<p class="p-num gray">${item.saledate}</p>
							<p class="p-amount">状态:	<c:if test="${item.state==0}" >待审核</c:if>
														<c:if test="${item.state==1}" >正常</c:if>

														<c:if test="${item.state==2}" >下架</c:if>
														<c:if test="${item.state==-1}" >审核不通过</c:if></p>
							<p class="p-amount">操作: &nbsp;&nbsp;&nbsp;
								<c:if test="${item.state==1}" >
									<a href="sellaction.do?action=modify&id=${item.id}" target="_self" class="btn" clstag="click|keycount|orderinfo|order_check">修改</a>
									<a href="sellaction.do?action=modifystate&set=2&id=${item.id}" target="_self" class="btn" clstag="click|keycount|orderinfo|order_check">下架</a>
								</c:if>
								<c:if test="${item.state==2}" >
									<a href="sellaction.do?action=modifystate&set=1&id=${item.id}" target="_self" class="btn" clstag="click|keycount|orderinfo|order_check">上架</a>
									<a href="sellaction.do?action=modifystate&set=3&id=${item.id}" target="_self" class="btn" clstag="click|keycount|orderinfo|order_check">删除</a>
								</c:if>
