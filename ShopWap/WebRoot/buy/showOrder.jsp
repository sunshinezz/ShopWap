		...
		<ul class="new-mu_l2w">
				<li class="new-mu_l2">
					<p class="new-tit new-p-re">
						<span class="new-txt">${order.itemName}</span>
						<span class="new-txt-rd2">￥${order.price }</span>
						<span class="new-txt-rd2 new-option-r" ><span class="new-icon2"></span></span> 
					</p>
					...
					<span class="new-mu_l2a new-p-re">
						<span class="new-mu_l2cw">			
							...
							使用余额：0.00元
							= 
							订单总金额：<strong class="h1">${order.price }</strong>
						</span>    
					</span>
					...
					<span class="new-mu_l2a new-p-re">
						<span class="new-mu_l2cw">
							收货人姓名：${order.name }<br />
							地址：${order.address }&nbsp; &nbsp;邮编：<br />						
							手机号码：${order.phone }   <br />  
						</span>    
						
						<div class="new-addr-btn">
							<c:if test="${order.state==0}">
							<a href="/ShopWap/pay/pay.do?action=subpay&orderid=${order.id}&totalprice=${order.price}" ><s></s>付款</a></td>
							</c:if>
							<c:if test="${order.state==2}">
							<a href="/ShopWap/buy/myOrderServlet.do?action=confirmBuy&orderId=${order.id }" ><s></s>确认收货</a></td>
							</c:if>
							<c:if test="${order.state==3}">
							<a href="/ShopWap/buy/myOrderServlet.do?action=tojudge&orderId=${order.id }"  ><s></s>评价</a></td>
							</c:if>
							<c:if test="${order.state==4||order.state==-1}">
							<a href="#" onclick="checkDel(${order.id})"><s></s>删除</a></td>
							</c:if>
							<c:if test="${order.state==-7}">
							<a href="/ShopWap/buy/myOrderServlet.do?action=gobackOrder&orderId=${order.id }" ><s></s>还原</a></td>
							</c:if>
